...
void alloc_port_map(const DWORD pid) {
	...
	if (process_port_map[pid] == nullptr) {
		spdlog::critical("Could not allocate memory for pid array");
		exit(1);
	}
	process_port_size[pid] = 0;
	pid_port_size_map_lock.unlock();
...
void update_tcp4() {
	...
	if (tcp_table == nullptr) {
		spdlog::warn("Error allocating memory");
		return;
	}
	...
	if (return_code == NO_ERROR) {
		...
		spdlog::warn("\tGetTcpTable2 failed with {}", return_code);
	}
}
...
void update_tcp6() {
	...
	if (return_code == NO_ERROR) {
		...
		spdlog::warn("\tGetTcp6Table2 failed with {}", return_code);
	}
}
...
void update_udp(const ULONG ul_af) {
	...
	if (return_code == NO_ERROR) {
		...
		spdlog::warn("\tGetExtendedUdpTable failed with {}", return_code);
	}
}
...
DWORD count_packets_for_pid(const DWORD pid, const UINT* port_packets) {
	...
	if (!get_connected_ports(pid, &ports)) {
		spdlog::warn("Could not get ports.");
		return 0;
	}
...
DWORD WINAPI start_packet_sniffing_loop(const LPVOID passed_thread_parameters) {
	...
	spdlog::info("Starting sniffing loop on {}", device_description);
	...
	if (device_description_copy == nullptr) {
		spdlog::warn("Could not allocate memory for copy.");
		return 0;
	}
	...
	if (p_loop_data == nullptr) {
		spdlog::warn("Could not allocate memory for pLoop_data");
		return 0;
	}
...
int start_packet_sniffing() {
	...
	while (dev_tmp != nullptr) {
		spdlog::info("------- {} -------", dev_tmp->description);
		...
		if ((dev_tmp->flags & PCAP_IF_LOOPBACK)
			...
			if (opened_handle != nullptr) {
				spdlog::info("Opened {}", dev_tmp->description);
				opened_handles[opened_handles_count] = opened_handle;
				opened_handles_count++;
			} else {
				spdlog::info("Could not open {}", dev_tmp->description);
			}
		}
		dev_tmp = dev_tmp->next;
	}
	...
	if (amount_of_devs == 0) {
		spdlog::warn("Could not find interface");
		return 2;
	}
	return 1;
}
...
pcap_t* sniff_device(pcap_if_t* device) {
	...
	if (data == nullptr) {
		spdlog::warn("Could not allocate memory for data.");
		return nullptr;
	}
	...
	if (handle == nullptr) {
		spdlog::warn("Couldn't open device {}: {}", device_name, errbuf);
		return nullptr;
	}
